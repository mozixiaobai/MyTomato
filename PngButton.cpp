...
//	SetWindowPos(NULL, 0, 0, rcButton.Width(), rcButton.Height(), SWP_NOACTIVATE|SWP_NOMOVE);  //这条语句不删除造成显示区域不全
}
...
bool CPngButton::ShowImage(CDC* pDC, Image* pImage, UINT nState)
{
	...
	if(pImage!=NULL)
	{
		...
		if (m_nImgPart == BTN_IMG_1)
		{
			int x=m_pImage->GetWidth();
			rcButton = CRect(0, 0, m_pImage->GetWidth(), m_pImage->GetHeight());
			bWidth = m_pImage->GetWidth();
		}
		else if(m_nImgPart == BTN_IMG_3)
		{
...
//				return false;
			...
			bWidth = m_pImage->GetWidth()/4;
		}
		else
			return false;
		
		Graphics graph(pDC->GetSafeHdc());
//      Rect destRect(0, 0, m_pImage->GetHeight(), m_pImage->GetHeight());     //原函数对于非正方形图像有显示bug
		Rect destRect(0, 0, bWidth, m_pImage->GetHeight());
		graph.DrawImage(pImage,destRect,rcButton.left, rcButton.top, rcButton.Width(), rcButton.Height(),UnitPixel);  //将图像指定值域绘制到指定位置指定大小
//		graph.DrawImage(pImage,0, 0, rcButton.left, rcButton.top, rcButton.Width(), rcButton.Height(), UnitPixel);
//		graph.DrawImage(pImage, 0, 0, 64,64);
...
bool CPngButton::SetFourState()
{
	...
	if(pImage!=NULL)
	{
		...
		Graphics graph(pDC->GetSafeHdc());
		Rect destRect(0, 0, m_pImage->GetHeight(), m_pImage->GetHeight());
		graph.DrawImage(pImage,destRect,rcButton.left, rcButton.top, rcButton.Width(), rcButton.Height(),UnitPixel);  //将图像指定值域绘制到指定位置指定大小
